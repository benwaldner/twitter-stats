...
def get_possibility_n():
    ...
    for key in CONSTS:
        if not CONSTS[key]['on']:
            assert 'default' in CONSTS[key], "if ON is False, then a default must be provided"
...
def sample_params(n = None, semi_random_params_key = 'conv_layer_n'):
    ...
    while i < n:
        ...
        for key in CONSTS:
            ...
            if 'values_at_position' in CONSTS[key]:
                values = [random.choice(candidates) for candidates in list(CONSTS[key]['values_at_position'].values())]
...
def format_params_to_cmd(name, params, 
                         ...
                         more_arguments = {}):
    arg_str = ' '.join(["--%s %r" %(k, v) for k, v in list(more_arguments.items())])
